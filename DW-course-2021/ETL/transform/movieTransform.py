...
movie_path_con = "D:/Code/CodeWareHouse/py/DW-course-data/movies.csv"


class MovieTransform:

    def __init__(self, movie_path=movie_path_con,
                 schema_path='D:/Code/CodeWareHouse/py/DW-course-data/movieReviewTransform'):
        print("MovieTransform ----- start ")
    ...
    def generate_day_schema(self):
        ...
        year, month, day = time_df['release_time'].dt.year, time_df['release_time'].dt.month, time_df[
            'release_time'].dt.day
        ...
        day_schema_zipped = day_schema.drop_duplicates(subset=['year', 'month', 'day'])[
            ['year', 'month', 'day', 'p_id']].copy()
        day_schema_zipped['day_id'] = range(day_schema_zipped.shape[0])
        print("day_schema_zipped")
        print(day_schema_zipped)

        day_schema = day_schema.merge(day_schema_zipped[['year', 'month', 'day', 'day_id']], how='outer',
                                      on=['year', 'month', 'day'])
        ...
        day_schema.to_csv(os.path.join(self.schema_path, 'day_schema.csv'), index=False)

        # insert day_id into movie.csv

        movie_schema = pd.read_csv(os.path.join(self.schema_path, 'movie_schema.csv'))[
            ['product_id', 'score', 'emotion_score']]
        ...
        movie_schema.to_csv(os.path.join(self.schema_path, 'movie_schema.csv'), index=False)
    ...
    def generate_version_schema(self):
        ...
        version_schema.to_csv(os.path.join(self.schema_path, 'version_schema.csv'), index=False)
    ...
    def generate_director_schema(self):
        ...
        director_count = director_schema['director'].value_counts().reset_index().rename(
            columns={'index': 'director', 'director': 'count'})
        director_schema = director_schema.merge(director_count, how='outer', on='director')
        print(director_schema)
        director_schema.to_csv(os.path.join(self.schema_path, 'director_schema.csv'), index=False)
    ...
    def generate_actor_schema(self):
        ...
        actor_count = actor_schema['actor'].value_counts().reset_index().rename(
            columns={'index': 'actor', 'actor': 'count'})
        actor_schema = actor_schema.merge(actor_count, how='outer', on='actor')
        print(actor_schema)
        actor_schema.to_csv(os.path.join(self.schema_path, 'actor_schema.csv'), index=False)
    ...
    def generate_label_movie(self):
        ...
        label_count = label_schema['label'].value_counts().reset_index().rename(
            columns={'index': 'label', 'label': 'count'})
        label_schema = label_schema.merge(label_count, how='outer', on='label')
        print(label_schema)
        label_schema.to_csv(os.path.join(self.schema_path, 'label_schema.csv'), index=False)
...
if __name__ == '__main__':
    assert os.path.exists(movie_path_con)
