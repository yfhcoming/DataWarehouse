...
reveiw_path_con = "D:/Code/CodeWareHouse/py/DW-course-data/reviews.csv"


class ReviewTransform:

    def __init__(self, review_path=reveiw_path_con,
                 schema_path='D:/Code/CodeWareHouse/py/DW-course-data/movieReviewTransform'):
        print("ReviewTransform -------- start ")
    ...
    def generate_user_schema(self):
        ...
        print(user_schema)
        user_schema.to_csv(os.path.join(self.schema_path, 'user_schema.csv'), index=False)
    ...
    def generate_review_schema(self):
        ...
        review_schema.to_csv(os.path.join(self.schema_path, 'review_schema.csv'), index=False)
        ...
        score_df = movie_schema['score'].value_counts().reset_index().rename(
            columns={'score': 'count', 'index': 'score'})
        ...
        score_schema.to_csv(os.path.join(self.schema_path, 'score_schema.csv'), index=False)
        ...
        emotion_score_df = movie_schema['emotion_score'].value_counts().reset_index().rename(
            columns={'emotion_score': 'count', 'index': 'emotion_score'})
        ...
        emotion_score_schema.to_csv(os.path.join(self.schema_path, 'emotion_score_schema.csv'), index=False)


if __name__ == '__main__':
    assert os.path.exists(reveiw_path_con)
    reviewTransform = ReviewTransform()
